...
// Error import
const _error_types_1 = require("./@error.types");
// Math class  import
...
class Matrix {
    ...
    constructor(entries, rows, columns) {
        ...
        if (!Array.isArray(entries)) {
            throw new _error_types_1.MatrixError("Matrix has to be an array", 801, { entries });
        }
        if (this.is1dArray(entries)) {
            if (rows === undefined || columns === undefined || typeof (rows) !== "number" || typeof (columns) !== "number" || columns <= 0 || rows <= 0) {
                throw new _error_types_1.MatrixError("Rows and columns must be defined for 1D array entries, rows and columns must be of type number and not be 0 or negative", 804);
            }
        ...
        if (typeof entries[0] === "object") {
            ...
            for (let i = 0; i < entries.length; i++) {
                if (this.getInterfaceName(entries[i]) !== typeName) {
                    throw new _error_types_1.MatrixError("Invalid entries not of the same type", 805, { entry: entries[i] });
                }
            }
        }
        else {
   ...
   * @param {number} row - The row index of the element starts from zero.
    ...
    */
    getElement(row, column) {
        if (typeof row !== "number" || typeof column !== "number")
            throw new _error_types_1.MatrixError("Invalid arugment", 606, { row, column });
        const index = row * this.columns + column;
        if (index > this.size || index < 0)
            throw new _error_types_1.MatrixError("Index out of bounds", 800, { row, column });
    ...
    getRow(rowIndex) {
        if (typeof rowIndex !== "number")
            throw new _error_types_1.MatrixError("Invalid argument", 606, { rowIndex });
        if (rowIndex <= 0 || rowIndex > this.rows)
            throw new _error_types_1.MatrixError("Row index out of bounds", 800, { rowIndex });
        const row = [];
    ...
    getColumn(columnIndex) {
        if (typeof columnIndex !== "number")
            throw new _error_types_1.MatrixError("Invalid argument", 606, { columnIndex });
        if (columnIndex <= 0 || columnIndex > this.columns)
            throw new _error_types_1.MatrixError("Column index out of bounds", 800, { columnIndex });
        const column = [];
    ...
    setElement(row, column, value) {
        if (typeof value !== typeof this.mElements[0] || typeof row !== "number" || typeof column !== "number")
            throw new _error_types_1.MatrixError("Invalid arugment", 606, { value, row, column });
        const index = row * this.columns + column;
        if (index > this.size || index < 0)
            throw new _error_types_1.MatrixError("Index out of bounds", 800, { row, column });
    ...
    */
    setSubMatrix(startRow, endRow, startCol, endCol, subMatrix) {
        if (startRow < 0 || startRow >= this.rows || endRow < 0 || endRow >= this.rows || startCol < 0 || startCol >= this.columns || endCol < 0 || endCol >= this.columns) {
            throw new _error_types_1.MatrixError("Invalid submatrix indices", 805);
        }
        ...
        if (subMatrixRows !== subMatrix.rows || subMatrixColumns !== subMatrix.columns) {
            throw new _error_types_1.MatrixError("Submatrix dimensions do not match", 806);
        }
    ...
    */
    ...
    add(B) {
        if (this.shape !== B.shape)
            throw new _error_types_1.MatrixError("Invalid matrix dimensions for addition", 805, { ARows: this.rows, AColumns: this.columns, BRows: B.rows, BColumns: B.columns });
        if (!(B instanceof Matrix))
            throw new _error_types_1.MatrixError("Argument is not an instance of Matrix", 804, { B });
        if (this.dataType !== B.dataType) {
            throw new _error_types_1.MatrixError("Matrices have different element types", 806, { AType: typeof this.mElements[0], BType: typeof B.mElements[0] });
        }
    ...
    gramSmith() {
        ...
        for (let i = 1; i < columns; i++) {
            ...
            for (let j = 0; j < i; j++) {
                ...
                let uv = math_1.default.dot(u, v);
                let uu = math_1.default.dot(u, u);
                let scalar = uv / uu;
            ...
            if ((Math.sqrt(orthogonalProjection.map(x => Math.pow(x, 2)).reduce((acc, x) => acc + x))) < constants_1.default.DELTA) {
                throw new _error_types_1.MatrixError("Cannot normalize a nearly-zero column. The given columns are not linearly independent.", 704);
            }
            orthogonalColumns.push(orthogonalProjection);
        }
        const normalizedColumns = orthogonalColumns.map((column) => math_1.default.normalize(column));
    ...
    multiply(B) {
        if (this.columns !== B.rows) {
            throw new _error_types_1.MatrixError("Invalid matrix dimensions for multiplication", 807, { rows: B.rows, columns: this.columns });
        }
    ...
    pow(exp) {
        if (!this.isSquare) {
            throw new _error_types_1.MatrixError("Can't multiply a non-square matrix with itself.", 810, { A: this.isSquare });
        }
        if (exp === 0) {
    ...
    scale(scalar) {
        if (typeof scalar !== "number")
            throw new _error_types_1.MatrixError("Invalid scalar", 606, { scalar });
    ...
    strassenMultiply(B) {
        if (!this.isSquare && !B.isSquare) {
            throw new _error_types_1.MatrixError("Both matrices has to be square", 810, { A: this.isSquare, B: B.isSquare });
        }
    ...
    subtract(B) {
        if (this.shape !== B.shape)
            throw new _error_types_1.MatrixError("Invalid matrix dimensions for subtraction", 805, { ARows: this.rows, AColumns: this.columns, BRows: B.rows, BColumns: B.columns });
        if (!(B instanceof Matrix))
            throw new _error_types_1.MatrixError("Argument is not an instance of Matrix", 804, { B });
        const resultElements = JSON.parse(JSON.stringify(this.mElements));
        const size = this.size;
    ...
    vMultiply(vector) {
        if (!Array.isArray(vector))
            throw new _error_types_1.MatrixError("The input vector is not an array", 606, { vector });
        if (vector.length !== this.columns)
            throw new _error_types_1.MatrixError("The length of the input vector must be equal to the number of columns in the matrix", 802, { matrixColumns: this.columns, vectorLength: vector.length });
    ...
    */
    ...
    backSubstitution(b) {
        if (!Matrix.isUpperTriangular(this))
            throw new _error_types_1.MatrixError("Matrix is not upper triangular", 815, { matrix: this });
        if (!Array.isArray(b))
            throw new _error_types_1.MatrixError("b is not an array", 606, { b });
        if (b.length !== this.rows)
            throw new _error_types_1.MatrixError("b does not have the same number of entries as the amount of rows of the matrix A", 802, { rowsOfA: this.rows, entriesOfb: b.length });
        const sol = [];
        const rows = this.rows;
        const columns = this.columns;
        for (let i = rows - 1; i >= 0; i--) {
            ...
            if (currentDiag === 0)
                throw new _error_types_1.MatrixError("Unsolvable system: zero on diagonal", 814, { matrix: this });
            let sum = 0;
    ...
    forwardSubstitution(b) {
        if (!Matrix.isLowerTriangular(this))
            throw new _error_types_1.MatrixError("Matrix is not lower triangular", 816, { matrix: this });
        if (!Array.isArray(b))
            throw new _error_types_1.MatrixError("b is not an array", 606, { b });
        if (b.length !== this.rows)
            throw new _error_types_1.MatrixError("b does not have the same number of entries as the amount of rows of the matrix A", 802, { rowsOfA: this.rows, entriesOfb: b.length });
    ...
    */
    ...
    augment(B) {
        if (!(B instanceof Matrix))
            throw new _error_types_1.MatrixError("Argument is not an instance of Matrix", 804, { B });
        if (B.rows !== this.rows)
            throw new _error_types_1.MatrixError("A and B does not have the same number of rows", 802, { ARows: this.rows, BRows: B.rows });
        let AA = this.toArray();
        let BA = B.toArray();
    ...
    */
    invertSquare() {
        if (!this.isSquare)
            throw new _error_types_1.MatrixError("Can't use this method for inverting a non square matrix, see the inverse method instead", 812, { isSquare: this.isSquare });
    ...
    invertUpper() {
        //TODO: Psudo inverse
        if (!this.isSquare)
            throw new _error_types_1.MatrixError("Can't use this method for inverting a non square matrix, see the inverse method instead", 812, { matrix: this });
        if (!Matrix.isUpperTriangular(this))
            throw new _error_types_1.MatrixError("Matrix is not upper triangular", 815, { matrix: this });
    ...
    invertLower() {
        ...
        if (!this.isSquare)
            throw new _error_types_1.MatrixError("Can't use this method for inverting a non square matrix, see the inverse method instead", 812, { matrix: this });
        if (!Matrix.isLowerTriangular(this))
            throw new _error_types_1.MatrixError("Matrix is not lower triangular", 815, { matrix: this });
        const identityMatrix = Matrix.identity(this.rows);
        let invertedMatrixElements = [];
    ...
    */
    ...
    equal(B) {
        ...
        if (B.dataType === "number") {
            return this.mElements.every((entry, index) => math_1.default.equal(entry, B.mElements[index]));
        }
    ...
    */
    ...
    static reshape(array, newRows, newColumns) {
        if (array.length !== newRows * newColumns)
            throw new _error_types_1.MatrixError("Invalid reshape dimensions", 806, { newRows, newColumns });
        if (!Array.isArray(array) || typeof newRows !== "number" || typeof newColumns !== "number")
            throw new _error_types_1.MatrixError("Invalid argument", 606, { array, newRows, newColumns });
        const newEntries = [];
        let rowIndex = 0;
        let colIndex = 0;
    ...
    */
    ...
    static padMatrixToPowerOfTwo(A) {
        ...
        const nextPower = math_1.default.nextPowerOfTwo(maxDimension);
     ...
     * @returns {void}
     */
    static roundMatrixToZero(A, threshold = constants_1.default.DELTA) {
        const size = A.size;
        for (let i = 0; i < size; i++) {
    ...
    static toFixedMatrix(A, digits, base = 10) {
        A.mElements = A.mElements.map((entry) => math_1.default.toFixedNumber(entry, digits, base));
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////
    /*
    ...
    */
    ...
    static identity(dimension) {
        if (dimension <= 0 || typeof dimension !== "number")
            throw new _error_types_1.MatrixError("Invalid argument", 606, { dimension });
        const entries = [];
    ...
    static ones(rows, columns) {
        if (rows <= 0 || columns <= 0 || typeof rows !== "number" || typeof columns !== "number")
            throw new _error_types_1.MatrixError("Invalid argument", 606, { rows, columns });
    ...
    */
    ...
    static isIntMatrix(A) {
        if (!(A instanceof Matrix))
            throw new _error_types_1.MatrixError("Argument is not an instance of Matrix", 804, { A });
